    ...
    <div class="bg-white border border-slate-200 rounded-xl shadow-sm">
        <div class="p-6 border-b border-slate-200 flex justify-between items-center">
            ...
            <form method="POST" action="{{ route('admin.scraper.run') }}">
                @csrf
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium flex items-center gap-2">
            ...
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50 text-slate-500 uppercase text-xs font-semibold">
                    <tr>
                        ...
                        <th class="px-6 py-4">ทรัพย์ที่พบ</th>
                        <th class="px-6 py-4">ทรัพย์ที่บันทึก</th>
                        <th class="px-6 py-4">เวลาที่ใช้</th>
                        <th class="px-6 py-4">วันที่ Scrape</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        <td class="px-6 py-4">
                            <a href="{{ $log->url }}" target="_blank" class="text-blue-600 hover:text-blue-700 text-xs truncate block max-w-xs">
                                {{ $log->url }}
                            </a>
                        </td>
                        <td class="px-6 py-4">
                            @if($log->status === 'success')
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700">สำเร็จ</span>
                            @elseif($log->status === 'failed')
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-700">ล้มเหลว</span>
                            @else
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-700">บางส่วน</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">{{ $log->properties_found ?? 0 }}</td>
                        <td class="px-6 py-4">{{ $log->properties_saved ?? 0 }}</td>
                        <td class="px-6 py-4">{{ $log->execution_time ?? 0 }}s</td>
                        <td class="px-6 py-4 text-slate-500">
                            {{ $log->last_scraped_at ? $log->last_scraped_at->format('d/m/Y H:i') : '-' }}
                        </td>
                        <td class="px-6 py-4">
                            @if($log->error_message)
                                <span class="text-xs text-red-600 truncate block max-w-xs" title="{{ $log->error_message }}">
